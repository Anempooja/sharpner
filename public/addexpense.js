function saveExpense(event){
    ...
    axios.post("https://35.154.166.226:4000/expense/addExpense",obj,{headers:{'Authorization':token}})
    .then(response=>{
...
window.addEventListener('DOMContentLoaded',()=>{
    // const token=localStorage.getItem('token')
    // axios.get("https://35.154.166.226:4000/expense/getExpense",{headers:{'Authorization':token}})
    // .then(response=>{
...
async function getProducts(page,rows){
    //console.log(page)
    const token=localStorage.getItem('token')
    await axios.get(`https://35.154.166.226:4000/expense/getExpense?page=${page}&rows=${rows}`,{headers:{'Authorization':token}})
    .then((response)=>{
...
document.getElementById('premium').onclick=async function(e){
    ...
    const token=localStorage.getItem('token')
    const response=await axios.get("https://35.154.166.226:4000/purchase/membership",{headers:{'Authorization':token}})
    ...
    var options={
        ...
        'handler':async function(response){
            axios.post("https://35.154.166.226:4000/purchase/updateTransactionStatus",{
...
if(childNodeToBeDeleted){
    ...
    await axios.delete(`https://35.154.166.226:4000/expense/${id}`,{headers:{'Authorization':token}})
    .then((response)=>{
...
btn.onclick=async()=>{
    const token=localStorage.getItem('token')
    const leaderBoard= await axios.get("https://35.154.166.226:4000/purchase/leaderboard",{headers:{'Authorization':token}})
...
async function downloadFile(event){
    ...
    event.preventDefault()
    const token=localStorage.getItem('token')
    await axios.get("https://35.154.166.226:4000/expense/download",{headers:{'Authorization':token}})
    .then((response)=>{
...
async function addDownloadedFiles(userId) {
  ...
  const response = await axios.get(
    "https://35.154.166.226:4000/expense/download",
    {
      headers: { Authorization: token },
    }
  );
